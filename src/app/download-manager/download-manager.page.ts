...
export class DownloadManagerPage
  implements DownloadManagerPageInterface, OnInit {
  headerObservable: any;
  _toast: any;
  storageInfo: AppStorageInfo;
  downloadedContents: Content[] = [];
  observations;
  defaultImg = this.commonUtilService.convertFileSrc(
  ...
  async ngOnInit() {
    ...
    return Promise.all([this.getDownloadedContents(true), this.getAppName()]);
  }
  ...
  private async getAppName() {
    return this.commonUtilService.getAppName().then((appName: any) => {
      this.appName = appName;
    });
  }

  private async getAppStorageInfo(): Promise<AppStorageInfo> {
    const req: ContentSpaceUsageSummaryRequest = {
      paths: [this.storageService.getStorageDestinationDirectoryPath()]
    };
    return this.contentService
      .getContentSpaceUsageSummary(req)
      .toPromise()
      ...
      .then(async data => {
        if (shouldGenerateTelemetry) {
          this.generateInteractTelemetry(
            data.length,
            this.storageInfo.usedSpace,
            this.storageInfo.availableSpace
          );
        }
        data.forEach(value => {
          value.contentData["lastUpdatedOn"] = value.lastUpdatedTime;
          value.contentData.appIcon = ContentUtil.getAppIcon(
            value.contentData.appIcon,
            value.basePath,
        ...
        if (this.db.pdb) {
          ...
          if (projectData.docs) {
            projectData.docs.sort(function(a, b) {
              return (
                new Date(b.updatedAt || b.syncedAt).valueOf() -
                new Date(a.updatedAt || a.syncedAt).valueOf()
              );
            });
            projectData.docs.map(doc => {
              doc.contentData = {
                lastUpdatedOn: doc.updatedAt,
                name: doc.title
              };
              doc.type = "project";
  ...
  private generateInteractTelemetry(
    ...
    valuesMap["spaceTakenByApp"] = this.commonUtilService.fileSizeInMB(
      usedSpace
    );
    valuesMap["freeSpace"] = this.commonUtilService.fileSizeInMB(
      availableSpace
    );
        ...
        .then(async (data: ContentDeleteResponse[]) => {
          ...
          if (data && data[0].status === ContentDeleteStatus.NOT_FOUND) {
            this.commonUtilService.showToast(
              this.commonUtilService.translateMessage("CONTENT_DELETE_FAILED")
            );
          } else {
  ...
  private async deleteAllContents(emitedContents) {
    const valuesMap = {};
    valuesMap["size"] = this.commonUtilService.fileSizeInMB(
      emitedContents.selectedContentsInfo.totalSize
    );
    ...
    this.deletedContentListTitle$ = new BehaviorSubject(
      "0/" + contentDeleteRequest.contentDeleteList.length
    );
    this.deleteAllConfirm = await this.popoverCtrl.create({
      component: SbPopoverComponent,
      componentProps: {
        sbPopoverHeading: this.commonUtilService.translateMessage(
          "DELETE_PROGRESS"
        ),
        actionsButtons: [
          {
  ...
  private async fetchStorageDestination() {
    this.storageDestination = await this.storageService
  ...
  private async presentPopupForLessStorageSpace() {
    this._toast = await this.popoverCtrl.create({
      ...
      componentProps: {
        ...
        sbPopoverMessage:
          this.storageDestination === StorageDestination.INTERNAL_STORAGE
            ? this.commonUtilService.translateMessage(
                "MOVE_FILES_TO_OTHER_DESTINATION",
  ...
  private checkAvailableSpace() {
    this.storageService
      .getStorageDestinationVolumeInfo()
      .pipe(
        tap(volumeInfo => {
