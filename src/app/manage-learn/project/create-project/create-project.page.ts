...
export class CreateProjectPage implements OnInit {
  ...
  constructor(
    ...
    route.queryParams.subscribe((parameters) => {
      ...
      console.log(parameters, "parameters inside if");
  ...
  onKeyPress(event, content) {
    console.log(event, "event", event.detail.data, content, this.projectForm.value[content.field]);
    ...
    if (event.detail) {
      ...
      if (str.match(emojiRegex, '')) {
        ...
        console.log(str, "str 80", this.projectForm.value[content.field]);
        this.projectForm.value[content.field]
      } else {
        console.log(str, "str 81 else");
      }
    }
          ...
          handler: () => {
            this.ngZone.run(() =>
              this.router.navigate([`${RouterLinks.PROJECT_OPERATION_PAGE}`, this.projectId], {
                queryParams: { availableInLocal: true, isEdit: false }, replaceUrl: true,
              })
            );
          }
        }
